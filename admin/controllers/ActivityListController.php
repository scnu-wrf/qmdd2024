...
class ActivityListController extends BaseController {
    ...
    public function actionIndex($project_id='', $keywords = '') {
        ...
        //$criteria->condition = 'state=721'; // 审核通过的
        $criteria->join = "JOIN activity_list_data t2 on t2.activity_id=t.id";
        if($project_id!=''){
            $criteria->join .= " and t2.project_id=".$project_id;
        }
        $criteria->condition = get_like($criteria->condition,'activity_code,activity_title',$keywords,''); // where语句
        $criteria->order = 'uDate DESC';
        $criteria->group='t2.activity_id';
        $data = array();
        $data['project_list'] = ClubProject::model()->findAll('state=371 and auth_state=461 and club_id='.get_session('club_id'));
    ...
    // 审核不通过
    public function actionIndex_not_pass($start_date = '', $end_date = '', $project_id='', $keywords = '') {
        set_cookie('_currentUrl_', Yii::app()->request->url);
        $modelName = $this->model;
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 'state=373 and '.get_where_club_project('activity_club_id','');
        $start_date=empty($start_date) ? date("Y-m-d") : $start_date;
        $end_date=empty($end_date) ? date("Y-m-d") : $end_date;
        if ($start_date != '') {
            $criteria->condition.=' and left(audit_time,10)>="' . $start_date . '"';
    ...
    // 活动管理
    public function actionIndexhd($index='', $start_date = '', $end_date = '', $project_id='', $keywords = '') {
        set_cookie('_currentUrl_', Yii::app()->request->url);
        $modelName = $this->model;
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 'state=2 and activity_club_id='.get_session('club_id');
        if($index==1){
            $criteria->condition .= ' and (dispay_end_time>=now() or sign_up_date_end>=now() or t.activity_time_end>=now())';
        }elseif($index==2){
            $criteria->condition .= ' and dispay_end_time<now() and sign_up_date_end<now() and t.activity_time_end<now()';
            $start_date=empty($start_date)?date("Y-m-d", strtotime("-1 month")):$start_date;
            $end_date=empty($end_date)?date("Y-m-d"):$end_date;
        }
        if ($start_date != '') {
            $criteria->condition.=' and left(t.activity_time,10)>="' . $start_date . '"';
        }
        if ($end_date != '') {
            $criteria->condition.=' and left(t.activity_time,10)<="' . $end_date . '"';
        }
        $criteria->join = "JOIN activity_list_data t2 on t2.activity_id=t.id";
        if($project_id!=''){
            $criteria->join .= " and t2.project_id=".$project_id;
    ...
    public function actionIndexhv($index='', $start_date = '', $end_date = '', $project_id='', $keywords = '') {
        ...
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 'state=2 and '.get_where_club_project('activity_club_id','');
        $start_date=empty($start_date)?date("Y-m-d"):$start_date;
        $end_date=empty($end_date)?date("Y-m-d"):$end_date;
        if($index==1){
            $criteria->condition .= ' and (dispay_end_time>=now() or sign_up_date_end>=now() or t.activity_time_end>=now())';
        ...
        $criteria->condition = get_like($criteria->condition,'activity_code,activity_title,activity_club_code,activity_club_name',$keywords,'');
        $criteria->order = 'uDate DESC';
        $criteria->group='t2.activity_id';
        $data = array();
        $data['start_date'] = $start_date;
    ...
    public function actionCreate() {
        ...
        if (!Yii::app()->request->isPostRequest) {
            ...
            $model->explain_temp=get_html($basepath->F_WWWPATH.$model->explain, $basepath);//获取html文件 
            $data['model'] = $model;
    ...
    public function save_data($model){
        ...
        if(!empty($_POST['add_tag'])){
            foreach($_POST['add_tag'] as $v){
                ...
                $namelist=array();//为了找到project_name 
                $namelist = ClubProject::model()->find('project_id='.$v['project_id']);//用对应的project_id找到project_name
                $data->activity_id = $model->id;
                $data->project_id = $v['project_id'];
                $data->project_name = $namelist->project_name;
    ...
    public function change_data($model){
        ...
        $cList->apply_way = $model->activity_apply_way_referee; // 未被定义
    ...
    public function actionIndex_pay_blueprint($start = '', $end = '',$keywords=''){
        ...
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = get_where_club_project('supplier_id').' and pricing_type=354 and if_del=648';
    ...
    public function actionPay_blueprint_details($id){
        ...
        if(!Yii::app()->request->isPostRequest){
            $data['model'] = $model;
            $data['mall_fee'] = ClubMembershipFee::model()->find('code="TS53"');
            $this->render('pay_blueprint_details',$data);
        }
    }

    // 报名费用统计
    public function actionIndex_pay_stat($start = '', $activity_title='', $end = '',$keywords=''){
        set_cookie('_currentUrl_', Yii::app()->request->url);
        $modelName = $this->model;
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 'activity_club_id='.get_session('club_id').' and state=2';
        $start=empty($start) ? date("Y-m-d") : $start;
        $end=empty($end) ? date("Y-m-d") : $end;
        if ($start != '') {
    ...
    public function actionIndex_video_live($keywords='') {
        ...
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 't.state=2 and !isnull(t.video_live_id)';
        $criteria->condition = get_like($criteria->condition,'t.activity_title',$keywords,'');
		$criteria->join = "JOIN video_live t2 on find_in_set(t2.id ,t.video_live_id)";
        $criteria->select='t.id,t.activity_title,t.video_live_id,t2.id video_id,t2.title video_title';
        $criteria->order = 't2.uDate';
        $data = array();
    ...
    public function actionRelieve($id,$video_id) {
        ...
        $criteria = new CDbCriteria;
        $criteria->condition = 'id in(' . $id . ')';
        $activity=$model->find($criteria);
        $arr=explode(",", $activity->video_live_id);
        if(!is_array($arr)){
            return $arr;
        }
        foreach($arr as $k=>$v){
    ...
    public function actionAddForm(){
        ...
        $model = $modelName::model();
        $criteria = new CDbCriteria;
        $criteria->condition = 'id in(' . $_POST['activity_id'] . ')';
    ...
    public function actiongetActivity($keywords){
        $criteria=new CDbCriteria;
        $criteria->addCondition('dispay_star_time<="'.date('Y-m-d-H-i-s').'"');
        $criteria->addCondition('dispay_end_time>="'.date('Y-m-d-H-i-s').'"');
        $criteria->addCondition('state_name="审核通过"');
        if($keywords!='')
        {
            $criteria->addSearchCondition('activity_title',$keywords);
        }
        $model=ActivityList::model()->findAll($criteria);
        $data=array();
        $i=0;
        foreach($model as $each){
            $data[$i]['list']=$each;
            $data[$i]['detail']=ActivityListData::model()->find('activity_id='.$each->id);
            $i++;
        }
        $i=0;
