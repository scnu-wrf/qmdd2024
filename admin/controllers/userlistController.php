...
class userlistController extends BaseController {
 ...
 //        }
 //        //$errors = array();
 //        show_status($st,$i,get_cookie('_currentUrl_'),$f);
 //    }
    ...
    function saveData($model,$post) {
        ...
        if(isset($post['GF_PASS'])){
            ...
            if(mb_strlen($post['GF_PASS'])<6){
                show_status(0,'',get_cookie('_currentUrl_'),'密码小于6位数，请输入长一些的密码');
                return;
            }
            //两次密码不一致
            if($post['GF_PASS']!=$post['GF_PASS2']){
                show_status(0,'',get_cookie('_currentUrl_'),'两次密码不一致');
                return;
            }
        }
