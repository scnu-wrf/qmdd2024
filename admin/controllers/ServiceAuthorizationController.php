<?php
    class ServiceAuthorizationController extends BaseController {
        ...
        public function actionIndex_move($keywords='',$back_url='') {
            ...
            if(!empty($keywords)){
                $criteria->condition .= ' and (service_type_name like "%'.$keywords.'%" or exists(select gu.GF_ID,gu.GF_ACCOUNT from userlist gu where gu.passed=2 and gu.GF_ID in(t.authorized_person_id) and (gu.GF_NAME like "%'.$keywords.'%" or gu.GF_ACCOUNT like "%'.$keywords.'%")))';
            }
            $data = array();
        ...
        public function actionGetChangeQuery($club_id,$service_type){
            $list = ServiceAuthorization::model()->find('club_id='.$club_id.' and service_type='.$service_type);
            echo (empty($list)) ? 0 : 1;
        }
    }