...
class ServiceGroupController extends BaseController {
    ...
    public function actionCustomer_service_list($keywords = '',$group_id="") {
		$data = array();
        $model = serviceGroupMember::model();
        $criteria = new CDbCriteria;
		$criteria->condition='club_id='.get_session('club_id');
	...
	public function actionAdd_group_member() {
		...
		$ids=$_POST["ids"];
        $data=array();
		Yii::app()->db->createCommand("UPDATE service_group_member,(select GROUP_CONCAT(g.id) as group_id from service_group g,service_group_member m  where find_in_set(g.id,m.group_id) and find_in_set(m.id,'{$ids}')) a SET service_group_member.group_id=concat_ws(',',a.group_id,'{$group_id}') WHERE find_in_set(id,'{$ids}')")->execute();
	...
	public function actionDel_group_member() {
		$group_id=$_POST["group_id"];
		$id=$_POST["id"];
		$member=serviceGroupMember::model()->find("id=".$id);
		...
		serviceGroupMember::model()->updateAll(array("group_id"=>$new_group_id),'id='.$id);
        $data=array();
        echo CJSON::encode($data);
    }
