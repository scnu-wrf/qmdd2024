<div class="box">
	...
	<div class="box-detail">
				...
				<tr>
                    ...
                    <td>
						...
						$.each(ProblemType,function(k,v){
							$.each($("#ClubServiceGroup_problem_type").val().split(","),function(m,n){
								if(n==v.id){
		...
		<div class="box-detail-bd" style="margin-top: 10px;">
			...
			<?php 
			$group_member=Yii::app()->db->createCommand("select a.*,b.admin_gfid,b.admin_gfaccount,b.admin_gfnick,b.is_on_line,b.last_login,GROUP_CONCAT(g.group_name) as group_name,u.ZSXM from gf_club_customer_service_group_member as a,qmdd_administrators as b,userlist as u,gf_club_customer_service_group g where find_in_set('{$model->id}',a.group_id) and a.admin_id=b.id and u.GF_ID=b.admin_gfid and find_in_set(g.id,a.group_id)")->queryAll();
			?>
	...
	$.dialog.open(url,{
		...
		close: function () {
			if($.dialog.data('problem_type')!=""){
				...
				$("#ClubServiceGroup_problem_type").val(problem_type.join());
			}
		}
	});
});
