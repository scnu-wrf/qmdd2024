     ...
     <?php  $form = $this->beginWidget('CActiveForm', get_form_list()); ?>
            ...
            <div style="display:block;" class="box-detail-tab-item">
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php $project_list=''; if($model->club_service_detailed1!=null){ $project_list=$model->club_service_detailed->project_id; }?>
                            <?php echo $form->hiddenField($model, 'project_list', array('class' => 'input-text', 'value'=>$project_list)); ?>
                            <span id="project_box"><?php if($model->club_service_detailed!=null){?><span class="label-box" id="project_item_<?php echo $model->club_service_detailed->project_id;?>" data-id="<?php echo $model->club_service_detailed->project_id;?>"><?php echo $model->club_service_detailed->project_list->project_name;?></span><?php }?></span>
                            <input style="display: none;" id="project_add_btn" class="btn" type="button" value="选择">
            ...
            <div class="box-detail-tab-item">
                ...
                <table id="timelist" class="showinfo" data-num="new">
                    ...
                    <tr class="item" data-id="new">
                        ...
                        <td><input style="width:80%;" class="input-text" type="text" value="" onclick="fnSetProductData(this);" readonly><input type="hidden" name="timelist[new][product_data_id]" value=""></td>
                    ...
                    <tr class="item" data-id="<?php echo $v->id;?>">
                        ...
                        <td><?php if($v->mall_product_data!=null){?><?php echo $v->mall_product_data->mall_products->name;?>(<?php echo $v->mall_product_data->json_attr;?>)<?php }?></td>
        ...
        <table class="showinfo">
            ...
            <tr>
                ...
                <td><?php if(isset($model->base_code->F_NAME)){ echo $model->base_code->F_NAME; }?></td>
...
// 项目添加或删除时，更新
var $ClubServiceData_project_list=$('#ClubServiceData_project_list');
...
var fnUpdateProject=function(){
    ...
    $ClubServiceData_project_list.val(we.implode(',',arr)).trigger('blur');
};
// 初始化时间，用于计算有效期限
...
var fnAddServiceDate=function(){
    ...
    var html='<tr class="item" data-id="'+timelist_num+'">'+
        ...
        '<td><input style="width:80%;" class="input-text" type="text" value="" onclick="fnSetProductData(this);" readonly><input class="service-date-product" type="hidden" name="timelist['+timelist_num+'][product_data_id]" value=""></td>'+
...
// 选择收费商品
var fnSetProductData=function(op){
    if(club_id<0){we.msg('minus', '请先选择发布单位'); return false;}
    $.dialog.data('product_id', 0);
    $.dialog.open('<?php echo $this->createUrl("select/productData");?>&club_id='+club_id,{
        id:'shangpin',
        lock:true,
        opacity:0.3,
        title:'选择具体内容',
        ...
        close: function () {
            if($.dialog.data('product_id')>0){
                $(op).val($.dialog.data('product_name')+'-'+$.dialog.data('product_attr')).siblings('input').val($.dialog.data('product_id'));
            }
        }
    });
};
...
$(function(){
    ...
    var fnResetDataId=function(){
        ...
        $ClubServiceData_club_id.val('');
        $project_box.html('');
        $ClubServiceData_project_list.val('');
    };
    // 更新服务者/场地
    var fnUpdateDataId=function(){
        ...
        if(type_id==174){
            ...
            $service_place_main.show();
            $project_add_btn.hide();
            if(service_person_project_id!=0){
                $project_box.html('<span class="label-box" id="project_item_'+service_person_project_id+'" data-id="'+service_person_project_id+'">'+service_person_project_name+'</span>');
            }
    ...
    $('#service_person_btn').on('click', function(){
        ...
        $.dialog.open('<?php echo $this->createUrl("select/servicePerson");?>'+'&club_id='+club_id,{
            id:'fuwuzhe',
            lock:true,
            opacity:0.3,
            title:'选择具体内容',
            ...
            close: function () {
                ...
                if($.dialog.data('service_person_id')>0){
                    ...
                    if($('#project_item_'+$.dialog.data('service_person_project_id')).length==0){
                       $project_box.append('<span class="label-box" id="project_item_'+$.dialog.data('service_person_project_id')+'" data-id="'+$.dialog.data('service_person_project_id')+'">'+$.dialog.data('service_person_project_name')+'</span>'); 
                       //fnUpdateProject();
    ...
    $service_place_btn.on('click', function(){
        ...
        $.dialog.open('<?php echo $this->createUrl("select/servicePlace");?>'+'&club_id='+club_id,{
            id:'changdi',
            lock:true,
            opacity:0.3,
            title:'选择具体内容',
    ...
    $project_add_btn.on('click', function(){
        if(service_place_id<=0){
            we.msg('minus','请先选择服务人/场地');
            return false;
        }
        $.dialog.data('project_id', 0);
        $.dialog.open('<?php echo $this->createUrl("select/project");?>&site_id='+service_place_id,{
            id:'xiangmu',
            lock:true,
            opacity:0.3,
            title:'选择具体内容',
