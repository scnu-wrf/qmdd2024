<div class="box">
  ...
  <?php  $form = $this->beginWidget('CActiveForm', get_form_list()); ?>
        ...
        <div class="box-detail-submit"><?php echo show_shenhe_box(array('baocun'=>'保存','shenhe'=>'提交审核'));?><button class="btn" type="button" onclick="we.back();">取消</button></div>
    <?php $this->endWidget(); ?>
    </div><!--box-detail end-->
</div><!--box end-->
<script>
...
var fnAddServiceDate=function(){
    ...
    var html='<tr class="item" data-id="'+timelist_num+'">'+
        ...
        '<td><input style="width:80%;" class="service-date-num input-text" type="text" name="timelist['+timelist_num+'][num]" value=""></td>'+
...
$(function(){
    ...
    $project_add_btn.on('click', function(){
        ...
        $.dialog.open('<?php echo $this->createUrl("select/project_list");?>&site_id='+service_place_id+'&club_id='+club_id,{
            id:'xiangmu',
            lock:true,
            opacity:0.3,
            title:'选择具体内容',
    ...
    $('#service_person_btn').on('click', function(){
		var clubid=$('#ClubServiceData_club_id').val();
        if(project_id<=0){
        ...
        $.dialog.open('<?php echo $this->createUrl("select/servicePerson");?>'+'&club_id='+clubid+'&project_id='+project_id,{
            id:'fuwuzhe',
            lock:true,
            opacity:0.3,
            title:'选择具体内容',
        ...
        $.dialog.open('<?php echo $this->createUrl("gfMaterial/materialPictureAll", array('type'=>252,'fd'=>135));?>&club_id='+club_id,{
            ...
            close: function () {
                if($.dialog.data('material_id')>0){
                    ...
                    $('#upload_pic_ClubServiceData_imgUrl').html('<a href="<?php echo $basepath->F_WWWPATH;?>'+art.dialog.data('app_icon')
