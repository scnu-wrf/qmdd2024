...
<div class="box">
    <div class="box-title c"><h1>商户管理》商户入驻》<?php if(isset($from))echo $from;?>商户》详情</h1>
    ...
    <div class="box-detail">
       <form id="myForm"  method="post" enctype="multipart/form-data">
            <table class="mt15" id="t2">
                ...
                <tr>
                    <td width="10%">商户类型</td>
                    <td width="40%"><?php echo $model->ct_type?></td>
                    <td width="10%">商户状态</td>
                    <td width="40%"><?php echo $model->ct_condition?></td>
                </tr>
                ...
                <tr>
                    ...
                    <td  width="40%"> 
                        ...
                        value="<?php echo $model->ct_certificates_start?>" data-title="营业执照有效期开始时间"  data-must="1" readonly> 
                        <div style="color:red"></div>
                    </td>
                       <script>
                ...
                <tr>
                    ...
                    <td width="40%" rowspan="3">
                        ...
                        <img id="imageDisplay2" src="<?php echo $model->ct_bank_pic?>" alt="上传的图片将在这里显示" style="width:50%;height: auto;" onclick="showModal(this)">
                    </td>                
                    <td width="10%">开户名称</td>
                    <td width="40%">
             ...
             <tr>
                ...
                <tr class="table-title">
                    <td colspan="4">商户<?php echo $model->ct_type=='个体工商户'?'经营者':'法人' ?>基本信息</td>
                </tr>
                <tr >
                 ...
                 <tr >
                    <td width="10%">名称</td>
                    <td width="40%">                     
                      <input type="text" class="input-text" name="ct_legal_entity_name" data-title="法人名称"  data-must="1"
                      value="<?php echo $model->ct_legal_entity_name?>">
                      <div style="color:red"></div>
                    </td>
                    <td width="10%">身份证号</td>
                    <td width="40%">
                      <input type="text" class="input-text" name="ct_legal_entity_idCardNumber" data-title="法人身份证号"  data-must="1" value="<?php echo $model->ct_legal_entity_idCardNumber?>" >
                      <div style="color:red"></div>
                    </td>
                </tr>
              <tr >
                    ...
                    <td width="10%">电子邮箱</td>
                    <td width="40%">
                      <input type="text" class="input-text email-input" name="ct_legal_entity_email" data-title="法人电子邮箱"  data-must="1" value="<?php echo $model->ct_legal_entity_email?>">
                ...
                <tr >
                    <td colspan="4">
                    <textarea id="myTextarea" name="ct_audit_reasons"style="width: 95%;resize: none;height: 300px;padding: 10px;font-size: 18px; direction: ltr;text-align: left;margin: auto;"><?php echo $model->ct_audit_reasons?>
                    </textarea>
                </td>
                </tr>
                <?php if($model->ct_condition=='已注销'){?>
                <tr class="table-title">
                    <td colspan="4">注销原因</td>
                </tr>
                <tr >
                    <td colspan="4">
                    <textarea id="myTextarea" name="ct_lock_reasons"style="width: 95%;resize: none;height: 300px;padding: 10px;font-size: 18px; direction: ltr;text-align: left;margin: auto;"><?php echo $model->ct_lock_reason?>
                    </textarea>
                </td>
                </tr>                    
                <?php }?>
               <tr class="table-title">
                ...
                <tr>
                    ...
                    <td colspan="3">
                    <input type="submit" value="保存" onclick="return check(3)" class="btn" style="background-color:#368ee0;color:white;">
                    <?php if($model->ct_condition=='待审核'){?>
                        <input type="submit" value="审核通过" onclick="return check(1)" class="btn" style="background-color:#368ee0;color:white;">
                        <input type="submit" value="审核不通过" onclick="return check(2)" class="btn" style="background-color:#368ee0;color:white;">
                    <?php }else if($model->ct_condition=='已过审'){?>
                        <input type="submit" value="审核不通过" onclick="return check(2)" class="btn" style="background-color:#368ee0;color:white;">
                    <?php }else if($model->ct_condition=='未过审'){?>
                        <input type="submit" value="审核通过" onclick="return check(1)" class="btn" style="background-color:#368ee0;color:white;">
                    <?php }else if($model->ct_condition=='已注销'){?>
                        <input type="submit" value="恢复" onclick="return check(4)" class="btn" style="background-color:#368ee0;color:white;">
                    <?php }?>
    ...
    <script>
        function submitForm(mode,url) {
            ...
            $.ajax({
                ...
                url: '/sports/admin/qmdd2018/index.php?r=commercialTenant/auditEnterprise&id=<?php echo $model->id?>&mode='+mode,  // 替换为实际的处理脚本地址
                data: formData,
...
<script type="text/javascript">
    function check(mode){
        ...
        // return true;
        if(mode==3){
            submitForm(3,'auditIndex');
        }
        if(mode==1){
        var result = confirm("确定“保存当前修改”并标记为“已过审”？");
        if(result){
          submitForm(1,'auditIndex');
        }else{
            return false;
        }
        }
        else if(mode==2){
        var result = confirm("确定“保存当前修改”并标记为“未过审”？");
        if(result){
        ...
        var result = confirm("确定“保存当前修改”并将当前‘已注销商户’标记为“待审核”？");
        if(result){
          submitForm(4,'lockIndex');
        }else{
            return false;
        }
        }
