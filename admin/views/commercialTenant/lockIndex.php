...
<script type="text/javascript">
    ...
    function sub(){
            ...
            $.ajax({
                ...
                url: '/sports/admin/qmdd2018/index.php?r=commercialTenant/unlockCT',  // 替换为实际的处理脚本地址
                data: formData,
                success: function(response) {
                    // 处理成功的响应
                      window.location.href = '/sports/admin/qmdd2018/index.php?r=commercialTenant/lockIndex';
                },
    ...
    #inner{
        ...
        <div id="outter" onclick="hid()">
            <div id="inner"  onclick="event.cancelBubble = true">
                <div id="lockTitle">注销原因</div>
                <hr>
                <form id="myForm">
                <div id="lockBody">
                    <textarea placeholder="输入注销原因（可选）" id="lockReason" name="lockReason" disabled></textarea>
                    <input type="text" value="" hidden id="hidId" name="hidId" >
                </div>
                </form>
                <button id="submitLockBtn"  onclick="sub()">恢复为待审核</button>
            </div>
        </div>
<div class="box">
    <div class="box-title c">
        <h1><?php if(empty($_REQUEST['edit_state'])){echo '当前界面：商户管理 》商户入驻 》已注销商户';}else{echo '当前界面：社区单位》单位认证管理 》单位认证申请';};?></h1>
...
<form action="/sports/admin/qmdd2018/index.php?r=commercialTenant/lockIndex" ><p>
<input type="hidden" name="r" value="commercialTenant/lockIndex" >
            ...
            <table class="list">
                <thead>
                    ...
                    <tr>
                        ...
                        onclick="location.href='/sports/admin/qmdd2018/index.php?r=commercialTenant/auditEnterprise&id=<?php echo $arc->id;?>&mode=0&from=3'" style="background-color:#368ee0;color:white;">查看</button>
                          <button class="btn" onclick="show(<?php echo $arc->id ?>,'<?php echo $arc->ct_lock_reason?>')">恢复</button>
