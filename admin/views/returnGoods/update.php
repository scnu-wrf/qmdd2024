    ...
    <?php  $form = $this->beginWidget('CActiveForm', get_form_list()); ?>
        <div class="box-detail-bd">
            <table style="table-layout:auto;">
                ...
                <tr>
                    ...
                    <td style="width:12%;"><?php echo $form->labelEx($model,'pay_type'); ?></td>
                    <td style="width:21.33%;"><?php if(!empty($model->orderinfo->pay_type_name))echo $model->orderinfo->pay_type_name; ?></td>
                    <td style="width:12%;"><?php echo $form->labelEx($model,'pay_time'); ?></td>
                    <td style="width:21.33%;"><?php if(!empty($model->orderinfo->pay_time))echo $model->orderinfo->pay_time; ?></td>
                </tr>
                <tr>
            ...
            <div class="mt15">
                ...
                <table id="goods_num" style="table-layout:auto;">
                    ...
                    ?>
                        <?php
                            if(!empty($order_data))foreach($order_data as $g) {
                                if($g->ret_state==372){
                                    $state=372;
                                }
                        ?>
                        <tr data-count="<?php echo $d_num; ?>">
                            ...
                            <input type="hidden" name="goods_num[<?php echo $d_num; ?>][ret_count]" value="<?php echo $g->ret_count; ?>">
                            ...
                            <td><?php echo $g->post; ?></td>
                            <td><?php echo $g->ret_count; ?></td>
                            <td><?php echo $g->buy_price*$g->ret_count; ?></td>
                            <td><input class="input-text" type="text" name="goods_num[<?php echo $d_num; ?>][ret_money]" value="<?php echo $g->ret_amount; ?>" <?php if($g->ret_state==372)echo 'readonly'; ?>></td>
                            <td>
                                <input id="ret_state_name_<?php echo $d_num; ?>" type="hidden" name="goods_num[<?php echo $d_num; ?>][ret_state_name]" value="<?php echo $g->ret_state_name; ?>">
                                <?php
                                    if($g->ret_state!=372){
                                        $f_id='372,373'; echo $form->dropDownList($g,'ret_state',Chtml::listData(BaseCode::model()->getReturn($f_id),'f_id','F_NAME'),array('prompt'=>'请选择','name'=>'goods_num['.$d_num.'][ret_state]','onchange'=>'optionRetstart(this,'.$d_num.')'));
                                    }
                                    else{
                                        echo $g->ret_state_name;
                                    }
                                ?>
                            </td>
                        </tr>
            ...
            <table class="mt15" style="table-layout:auto;">
                ...
                <tr>
                    ...
                    <td style="width:33.35%;"><?php echo ($model->orderdata->ret_state!=372) ? $form->textField($model,'return_club_name',array('class'=>'input-text')) : $model->return_club_name; ?></td>
                    <td style="width:16.65%;"><?php echo $form->labelEx($model,'return_club_tel'); ?></td>
                    <td style="width:33.35%;"><?php echo ($model->orderdata->ret_state!=372) ? $form->textField($model,'return_club_tel',array('class'=>'input-text')) : $model->return_club_tel; ?></td>
                </tr>
                <tr>
                    <td><?php echo $form->labelEx($model,'return_club_address'); ?></td>
                    <td><?php echo ($model->orderdata->ret_state!=372) ? $form->textField($model,'return_club_address',array('class'=>'input-text')) : $model->return_club_address; ?></td>
                    <td><?php echo $form->labelEx($model,'return_club_mail_code'); ?></td>
                    <td><?php echo ($model->orderdata->ret_state!=372) ? $form->textField($model,'return_club_mail_code',array('class'=>'input-text')) : $model->return_club_mail_code; ?></td>
                </tr>
            </table>
            <table class="mt15" style="table-layout:auto;">
                <tr class="table-title">
                ...
                ?>
                <tr>
                    <td style="width:12%;"><?php echo $form->labelEx($model,$logistics); ?></td>
                    <td style="width:21.33%;"><?php echo ($ct==1137) ? $model->ret_logistics : $model->change_no; ?></td>
                    <td style="width:12%;"><?php echo $form->labelEx($model,$logistics_name); ?></td>
                    <td style="width:21.33%;"><?php echo ($ct==1137) ? $model->ret_logistics_name : $model->change_logistics_name; ?></td>
                    <td style="width:12%;"><?php echo $form->labelEx($model,'change_date'); ?></td>
                    <td style="width:21.33%;"><?php echo $model->change_date; ?></td>
                </tr>
            </table>
        ...
        <div class="mt15">
            <table style="table-layout:auto;">
                ...
                <tr>
                    <td style="width:16.7%;"><?php echo $form->labelEx($model, 'after_sale_state'); ?></td>
                    <td style="width:;" colspan="3">
                        <?php
                            ...
                            if($ct==1137){
                                ...
                                $fid = '1150,1151,1152,1287,1288,1155,1156';
                                $whe = " order by FIND_IN_SET(`f_id`,'".$fid."')";
                            }
                            // else{
                            //     $fid = '1150,1151,1152,1153,1154,1155,1156,1287,1288';
                            // }
                            
                            // $fid = '1150,1153,1156';
                            // $whe = " order by FIND_IN_SET(`f_id`,'".$fid."')";
                            $disabled = '';
                            if($model->after_sale_state==1155 || $model->after_sale_state==1156){
                        ...
                        ?>
                        <select name="ReturnGoods[after_sale_state]" id="ReturnGoods_after_sale_state">
                            <?php if(!empty($basecode))foreach($basecode as $b) {?>
                                <option value="<?php echo $b->f_id; ?>" disabled="<?php echo $disabled; ?>"><?php echo $b->F_NAME; ?></option>
                            <?php }?>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="3">
                        <?php echo ($model->orderdata->ret_state!=372) ? $form->textField($model, 'desc', array('class' => 'input-text')) : $model->desc; ?>
                ...
                <tr>
                	<td>可执行操作</td>
                    <td colspan="3">
                    	<?php if($model->after_sale_state!=1155 || $model->after_sale_state!=1156)echo show_shenhe_box(array('baocun'=>'保存')); ?>
