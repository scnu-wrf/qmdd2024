   ...
   <div class="box">
    <div class="box-title c">
        <h1>当前界面：商城》订单管理》订单列表》<a class="nav-a">详情</a></h1>
...
var addHtml='<div style="width:500px;">'+
'<table class="box-detail-table showinfo">'+
    '<tr>'+
        '<td width="20%">物流公司</td>'+
        '<td><input id="dialog_logistics_id" type="hidden" value=""><span id="logistics_box"></span><input type="button" onclick="fnSelectLogistics();" class="btn" value="选择物流" /></td>'+
    '</tr>'+
    '<tr>'+
        '<td>物流单号</td>'+
        '<td><input id="dialog_logistics_number" type="text" class="input-text" ></td>'+
    '</tr>'+
'</table>'+
'</div>';
function fnDeliver(){
...
// 发货
var fnAdddeliver=function(data_id){
    var logistics_id=$('#dialog_logistics_id').val();
    var logistics_number=$('#dialog_logistics_number').val();
    if(logistics_id==''){
        we.msg('minus', '请先选择物流公司');
        return false;
    }
    we.loading('show');
    $.ajax({
        type: 'post',
        url: '<?php echo $this->createUrl('Deliver');?>',
        data: {logistics_id: logistics_id, logistics_number:logistics_number,data_id:data_id},
        dataType: 'json',
        success: function(data) {
            if(data.status==1){
                we.loading('hide');
                $.dialog.list['addDeliver'].close();
                we.success(data.msg, data.redirect);
            }else{
                we.loading('hide');
                we.msg('minus', data.msg);
