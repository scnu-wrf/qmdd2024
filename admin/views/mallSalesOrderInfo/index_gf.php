<div class="box">
    <div class="box-title c">
        <h1>当前界面：财务》订单管理》<a class="nav-a">订单列表</a></h1>
...
foreach($arclist as $v){ ?>
                    <tr>
                        ...
                        <td><?php 
                            $orderdata=MallSalesOrderData::model()->findAll('info_id='.$v->id);
                            $tx='';
                            $o_num=count($orderdata);
                        if($v->order_type==361 || $v->order_type==355 || $v->order_type==357){
                            if($o_num>1){
                            ...
                            if($o_num>1){
                                $tx=$orderdata[0]['service_name'].','.$orderdata[0]['service_data_name'].','.$orderdata[0]['buy_count'].'<br>...';
                            } elseif ($o_num==1) {
                                $tx=$orderdata[0]['service_name'].','.$orderdata[0]['service_data_name'].','.$orderdata[0]['buy_count'];
                            }
                        }
                            echo $tx; ?></td>
                        ...
                        <td>
                            <a class="btn" href="<?php echo $this->createUrl('update_gf', array('id'=>$v->id));?>" title="详情">查看</a>
