    ...
    <div class="material c">
        <div class="material-lt">
            ...
            <div class="material-video c">
                ...
                <?php $basepath=BasePath::model()->getPath(177); ?>
                    <?php foreach($arclist as $v){ ?>
                    <?php $str=substr($v->v_pic,0,6); ?>
                        <?php if($str=='record') {
							$pic_path=$v->v_file_path;
						} else {
							$pic_path=$basepath->F_WWWPATH;} ?>
                    <li>
                        <div class="pic">
                        <?php //echo $v->v_file_path;?><?php //echo $v->v_name;?>
                        <a href="<?php echo $this->createUrl('video_player',array('id'=>$v->id));?>" target="_blank">
                        <img src="<?php echo $pic_path;?><?php if($v->v_pic!='') { echo $v->v_pic; } else {?>2018/05/15/09/95_ap_238__150943450403.jpg<?php } ?>"></a></div>
                        <div class="title">
        ...
        <div class="material-rt">
            <div class="material-group">
                ...
                <a<?php if($group_id==$v->id){?> class="current"<?php }?> href="<?php echo $this->createUrl('gfMaterial/'.$action, array('group_id'=>$v->id));?>"><?php echo $v->group_name;?><span>(<?php echo GfMaterial::model()->getNum($v->id,253);?>)</span><span class="hidden_del" onclick="fnDeleteGroup(<?php echo $v->id;?>);"></span></a>
                <?php } ?>
            </div>
...
var fnAddGroup=function(){
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo $this->createUrl('addGroup',array('m_type'=>253));?>',
...
var fnChangeGroupShow=function(id,op){
    ...
    $.ajax({
         type:'GET',
         url:'<?php echo $this->createUrl('getGroup',array('m_type'=>253));?>',
         dataType:'html',
         success:function(data){
