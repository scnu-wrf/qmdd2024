    ...
    <div class="material c">
        <div class="material-lt">
            <div class="material-hd c">
                <div class="material-hd-lt">全部图片</div>
                <div class="material-hd-rt">
                    <div class="material-upload"><script>var materialPicUrl='<?php echo $this->createUrl('gfMaterial/uppic', array('group_id'=>$group_id));?>';we.materialPic();</script></div>
                    <span class="msg">大小不超过2M</span>
                </div>
            </div><!--material-hd end-->
            ...
            <div class="material-pic c">
                <ul id="material-main" class="c">
                    <?php foreach($arclist as $v){ ?>
                    <li>
                        <div class="pic"><a href="<?php echo $v->v_file_path;echo (empty($v->v_name)) ? $v->v_pic : $v->v_name;?>" target="_blank"><img src="<?php echo $v->v_file_path;echo (empty($v->v_name)) ? $v->v_pic : $v->v_name;?>"></a></div>
                        <div class="title">
        ...
        <div class="material-rt">
            <div class="material-group">
                <a<?php if($group_id==0){?> class="current"<?php }?> href="<?php echo $this->createUrl('gfMaterial/'.$action);?>">全部图片<span>(<?php echo $all_num;?>)</span></a>
                ...
                <a<?php if($group_id==$v->id){?> class="current"<?php }?> href="<?php echo $this->createUrl('gfMaterial/'.$action, array('group_id'=>$v->id));?>"><?php echo $v->group_name;?><span>(<?php echo GfMaterial::model()->getNum($v->id,252);?>)</span><span class="hidden_del" onclick="fnDeleteGroup(<?php echo $v->id;?>);"></span></a>
                <?php } ?>
            </div>
...
var fnAddGroup=function(){
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo $this->createUrl('addGroup',array('m_type'=>252));?>',
...
var fnChangeGroupShow=function(id,op){
    ...
    $.ajax({
         type:'GET',
         url:'<?php echo $this->createUrl('getGroup',array('m_type'=>252));?>',
         dataType:'html',
         success:function(data){
