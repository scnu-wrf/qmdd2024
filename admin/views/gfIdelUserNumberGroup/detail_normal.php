<div class="box">
    <div class="box-title c"><h1><i class="fa fa-table"></i>普通号码详情</h1><span class="back"><a class="btn" href="javascript:;" onclick="we.back();"><i class="fa fa-reply"></i>返回</a></span></div><!--box-title end-->
            ...
            <div style="display:block;" class="box-detail-tab-item">
                <table id="num_tab">
					<tr>
						...
						<td colspan="4">数量：<?php echo $model->total_count;?>个（红色上标为VIP号，灰色底为已注册）</td>
						<td colspan="3">普通号：<?php echo $model->nomal_count;?>个</td>
						<td colspan="3">VIP号：<?php echo $model->vip_count;?>个</td>
					</tr>
					<?php 
...
<script>
	...
	$(function(){
		...
		$.each(all_num_arr,function(k,v){
			...
			if(v["f_vip"]==1){
				content+='<td style="position:relative;'+(v["is_use"]==1?'background:#ddd;':'')+'">'+v["account"].substr(4)+'<span style="position:absolute;color:red;font-size:smaller;top: 0;right: 5px;">'+v["level_name"]+'</span></td>';
			}else{
				content+='<td'+(v["is_use"]==1?' style="background:#ddd;"':'')+'>'+v["account"].substr(4)+'</td>';
			}
