        ...
        <div class="box-search">
            <form action="<?php echo Yii::app()->request->url;?>" method="get">
                ...
                <label style="margin-right:20px;">
                    <span>培训标题：</span>
                    <?php echo downList($train_list,'id','train_title','title','onchange="changeSignData(this);"'); ?>
                </label>
                <label style="margin-right:20px;">
                    <span>培训内容：</span>
                    <!-- <select name="content"><option value=''>请选择</option></select> -->
                    <?php echo downList($train_data,'id','train_content','content'); ?>
                </label>
                <label>
                    <span>关键字：</span>
            ...
            <table class="list">
                <thead>
                    <tr>
                        ...
                        <th><?php echo $model->getAttributeLabel('train_data_id');?></th>
                ...
                <tbody>
                    <?php $index = 1; foreach($arclist as $v){ ?>
                        <tr>
                            ...
                            <td><?php echo $v->train_data_content; ?></td>
...
<script>
    var data=<?php echo json_encode(toArray($train_list,'id,train_title'));?>
</script>
<script>
            ...
            '<div id="add_format" style="width:500px;">'+
                '<form id="add_form" name="add_form">'+
                    '<table class="list" id="table_tag" style="width:100%;border: solid 1px #d9d9d9;">'+
                        '<tr>'+
                            '<td colspan="2" style="width:25px;padding: 5px;">培训标题&nbsp;&nbsp;</td>'+
                            '<td>'+
                                '<select onchange="changeSignData(this,1);" id="train_id">'+
                                '<option value="">请选择</option>';
                                $.each(data,function(k,info){
                                    add_html+='<option value="'+info.id+'">'+info.train_title+'</option>'
                                })
                                add_html+='</select>'+
                            '</td>'+
                        '</tr>'+
                        '<tr>'+
                            '<td colspan="2" style="width:25px;padding: 5px;">培训选择&nbsp;&nbsp;</td>'+
                            '<td>'+
                                '<select id="train_data">'+
                                    '<option value="">请选择</option>';
                                add_html+='</select>'+
                            '</td>'+
                        '</tr>'+
            ...
            button:[
                {
                    ...
                    callback:function(){
                        if($("#train_id").val()==''){
                            we.msg('minus', '请选择培训标题');
                            return false;
                        }
                        if($("#train_data").val()==''){
                            we.msg('minus', '请选择培训内容');
                            return false;
                        }
                        window.location.href="<?php echo $this->createUrl('create'); ?>&train_id="+$("#train_id").val()+'&train_data_id='+$("#train_data").val();
                    },
    ...
    function changeSignData(obj,index) {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data)
                $.each(data,function(k,info){
                    content+='<option value="'+info.id+'" remnant="'+info.remnant+'" >'+info.train_content+'</option>'
                })
                if(index==1){
                    $("#train_data").html(content);
                }else{
