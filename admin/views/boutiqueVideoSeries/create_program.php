...
<div class="box">
    <div class="box-title c"><h1>当前界面：视频》视频分集管理》发布视频分集》<a class="nav-a">添加</a></h1><span class="back"><a class="btn" href="javascript:;" onclick="we.back();"><i class="fa fa-reply"></i>返回上一层</a></span></div><!--box-title end-->
		...
		<div class="box-detail-bd">
			<table>
				<tr>
                    <td width="15%">选择视频</td>
                    <td>
						<input class="input-text" readonly="readonly" style="width:200px;" id="video_title" type="text" value="" autocomplete="off">
			...
			<?php echo $form->error($model, 'video_id', $htmlOptions = array()); ?>
...
$('#video_select_btn').on('click', function(){
	$.dialog.open('<?php echo $this->createUrl("customer_service_list");?>&club_id=<?php echo get_session("club_id")?>',{
		id:'fuwuzhe',lock:true,opacity:0.3,width:'800px',height:'65%',
		title:'选择视频',
		close: function () {
			$("#BoutiqueVideoSeries_video_id").val($.dialog.data('video_id'));
			$("#video_title").val($.dialog.data('video_title'));
			$("#publish_classify_box").html($.dialog.data('publish_classify_name'));
		}
	});
})
// 选择视频
