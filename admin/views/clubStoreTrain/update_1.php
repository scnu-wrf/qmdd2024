<?php
    ...
    $cr='service_type=352 and service_id='.$model->service_id.' and id<>'.$model->id.' and change_time<"'.$model->change_time.'" and change_time order by change_time DESC';
    $up=ClubChangeList::model()->find($cr);
?>
<?php
    $_REQUEST['type_id']=empty($_REQUEST['type_id'])?$model->type_id:$_REQUEST['type_id'];
    $t_type=ClubStoreType::model()->find('id='.$_REQUEST['type_id']);
?>
<div class="box">
    <div class="box-title c">
        <h1>当前界面：培训/活动》培训管理》培训信息更改》详情</h1>
        ...
        <div class="box-detail-bd">
            <div style="display:block;" class="box-detail-tab-item">
                <table style="table-layout:auto;">
                    ...
                    <tr>
                        <td class="<?= $up->type_id!=$model->type_id?'red':''?>" width="10%;">
                            <?php echo $form->labelEx($model, 'type_id'); ?>
                        </td>
                        <td class="<?= $up->type_id!=$model->type_id?'red':''?>" width="40%" colspan="3">
                            <?php echo '<span class="label-box">'.$model->type_name.'</span>';?>
                        </td>
                    </tr>
                    <tr>
                        <td width="10%;">
                    ...
                    <tr>
                        <td <?= $up->start_time!=$model->start_time||$up->end_time!=$model->end_time?'red':''?>>培训时间</td>
                        <td <?= $up->start_time!=$model->start_time||$up->end_time!=$model->end_time?'red':''?>><?php echo $model->start_time.'-'.$model->end_time; ?></td>
                        <td class="<?= $up->dispay_start_time!=$model->dispay_start_time||$up->dispay_end_time!=$model->dispay_end_time?'red':''?>">前端显示时间</td>
                        <td class="<?= $up->dispay_start_time!=$model->dispay_start_time||$up->dispay_end_time!=$model->dispay_end_time?'red':''?>"><?php echo $model->dispay_start_time.'-'.$model->dispay_end_time; ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="<?= $up->men!=$model->men?'red':''?>"><?php echo $form->labelEx($model, 'men'); ?></td>
                        <td class="<?= $up->men!=$model->men?'red':''?>"><?php echo $model->men ?></td>
                        <td class="<?= $up->phone!=$model->phone?'red':''?>"><?php echo $form->labelEx($model, 'phone'); ?></td>
                        <td class="<?= $up->phone!=$model->phone?'red':''?>"><?php echo $model->phone ?></td>
                    </tr>
                    <?php if($t_type->f_id==1504){?>
                        <tr>
                            <td class="<?= $up->organizational!=$model->organizational?'red':''?>"><?php echo $form->labelEx($model, 'organizational'); ?></td>
                ...
                <div class="mt15" id="train_data">
                    ...
                    <table class="mt15 train_data <?= empty($ud)?'red':'';?>" data_index="<?= $num;?>" style="table-layout:auto;">
                        ...
                        <tr>
                            <td class="<?= !empty($ud)&&$ud->type_id<>$d->type_id?'red':'';?>" style="width:10%;">类别</td>
                            <td class="<?= !empty($ud)&&$ud->type_id<>$d->type_id?'red':'';?>" style="width:40%;"><?php echo $d->type_name; ?></td>
                            <td class="<?= !empty($ud)&&$ud->project_id<>$d->project_id?'red':'';?>" style="width:10%;">项目</td>
                            <td class="<?= !empty($ud)&&$ud->project_id<>$d->project_id?'red':'';?>" style="width:40%;"><?php echo $d->project_name; ?></td>
                        </tr>
                        <tr>
                            <td class="<?= !empty($ud)&&$ud->content<>$d->content?'red':'';?>">培训内容</td>
                        ...
                        <tr>
                            <td class="<?= !empty($ud)&&$ud->money<>$d->money?'red':'';?>">培训费用（元）</td>
                            <td class="<?= !empty($ud)&&$ud->money<>$d->money?'red':'';?>"><?= $d->money;?></td>
                            <?php 
                                $cl='';
                                if(!empty($ud)){
                                    if($t_type->f_id==1504&&$ud->apply_check_way<>$d->apply_check_way){
                                        $cl='red';
                                    }elseif($t_type->f_id!=1504&&$ud->period<>$d->period){
                                        $cl='red';
                                    }
                                }
                            ?>
                            <td class="<?= $cl;?>"><?php echo $t_type->f_id==1504?'报名审核方式':'培训周期' ?></td>
                            <td class="<?= $cl;?>">
                                <?php if($t_type->f_id==1504){?>
                        ...
                        <tr>
                            <?php if($t_type->f_id==1504){?>
                                <td class="<?= !empty($ud)&&($ud->start_time<>$d->start_time||$ud->end_time<>$d->end_time)?'red':'';?>">培训时间</td>
                            ...
                            <td class="<?= !empty($ud)&&($ud->min_age<>$d->min_age||$ud->max_age<>$d->max_age)?'red':'';?>" <?php echo $t_type->f_id!=1504?'colspan="3"':'' ?>>
                                <?php echo $d->min_age.'('.$model->getAge(strtotime($d->min_age)).'周岁)-'.$d->max_age.'('.$model->getAge(strtotime($d->max_age)).'周岁)';?>
                            </td>
                        </tr>
                        <?php if($t_type->f_id==1504){?>
                            <tr>
                                <td class="<?= !empty($ud)&&($ud->train_identity_type<>$d->train_identity_type||$ud->train_identity_rank<>$d->train_identity_rank)?'red':'';?>">职称等级</td>
                                <td class="<?= !empty($ud)&&($ud->train_identity_type<>$d->train_identity_type||$ud->train_identity_rank<>$d->train_identity_rank)?'red':'';?>" colspan="3">
                                    <?php echo $d->train_identity_type_name.'-'.$d->train_identity_rank_name; ?>
                                </td>
                            </tr>
                        <?php }?>
                    </table>
