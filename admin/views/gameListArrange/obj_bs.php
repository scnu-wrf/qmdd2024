...
<div class="box">
    <div class="box-detail">
        ...
        <div class="box-detail-bd">
            <div id="form_div">
                <table id="game_table" class="box-detail-table">
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <?php
                                $bureau = '';
                                $bureau_num = '';
                                //if(!empty($model->bureau_num)){
                                    $bureau = explode(',',$model->bureau_num);
                                   // $bureau_num = explode('胜',$bureau[1]);
                                    //$bureau = $bureau[0];
                                   // $bureau_num = $bureau_num[0];
                                //}
                            ?>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><?php echo $form->textField($model,'star_time',array('class'=>'input-text1')); ?></td>
                    ...
                    <tr>
                        <?php
                            $arrange = GameListArrange::model()->findAll('game_data_id='.$model->game_data_id.' and left(arrange_tcode,7)="'.$model->arrange_tcode.'" and length(arrange_tcode)=9');
                        ...
                        ?>
                            <td rowspan="1" class="bold">参赛者</td>
                            <td colspan="3">
                    ...
                    <tr>
                        <td class="bold">局比分</td>
                        <td colspan="3"><input type="text" class="input-text score_style onkeyup" id="bureau_score_0" name="bureau_score_0" value="<?php echo $arrange[0]->bureau_score; ?>"></td>
                        <td class="blue_vs">:</td>
                        <td colspan="3"><input type="text" class="input-text score_style onkeyup" id="bureau_score_1" name="bureau_score_1" value="<?php echo $arrange[1]->bureau_score; ?>"></td>
                    </tr>
                    <tr>
                        <td class="bold">单杆分</td>
                        <td colspan="3"><input type="text" class="input-text score_style onkeyup" id="single_score_0" name="single_score_0" value="<?php echo $arrange[0]->single_score; ?>"></td>
                        <td class="blue_vs">:</td>
                        <td colspan="3"><input type="text" class="input-text score_style onkeyup" id="single_score_1" name="single_score_1" value="<?php echo $arrange[1]->single_score; ?>"></td>
                    </tr>
                    <tr class="not_padding not_sele">
                        <td rowspan="2" class="bold">输分区</td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,1,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:red;">红球</div>
                            <span style="font-size: 24px;">1</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,2,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:yellow;color:black;">黄球</div>
                            <span style="font-size: 24px;">2</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,3,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:green;">绿球</div>
                            <span style="font-size: 24px;">3</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,4,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:brown;">棕球</div>
                            <span style="font-size: 24px;">4</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,5,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:blue;">蓝球</div>
                            <span style="font-size: 24px;">5</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,6,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:Pink;color:black;">粉球</div>
                            <span style="font-size: 24px;">6</span>
                        </td>
                        <td onclick="clickAddnum(<?php echo $model->id; ?>,7,901);" style="cursor: pointer;">
                            <div style="padding: 5px;font-size: 18px;background-color:black;">黑球</div>
                            <span style="font-size: 24px;">7</span>
                        </td>
                    </tr>
                    <tr class="not_sele">
                        <td colspan="3" onclick="clickChange(901);" style="cursor: pointer;background-color: #EBF1DE;"><span class="bold" style="font-size: 18px;">换击球方</span></td>
                        <td colspan="4" onclick="clickRevoke(901);" style="cursor: pointer;background-color: #EBF1DE;padding: 10px;font-size: 20px;font-weight: bold;">撤销</td>
                        <!--弃用 <td colspan="2" onclick="clickRecovery(901);" style="cursor: pointer;background-color: #EBF1DE;padding: 10px;font-size: 20px;font-weight: bold;">恢复</td> -->
                    </tr>
                    <tr class="not_sele">
                        <td class="bold">操作</td>
                        <td colspan="3" id="readyGame" style="cursor: pointer;background-color:#EBF1DE;font-size: 20px;font-weight: bold;" onclick="onTime(this);">本场比赛开始</td>
...
<script>
    ...
    function onTime(obj){
        ...
        save_ajax(<?php echo $model->game_over; ?>);
        // 更改比赛状态
    ...
    // 加分
    function clickAddnum(id,num,game_over){
        $('.span_redio').each(function(){
            if($(this).css('display')=='inline-block'){
                var c=1;
                if($(this).attr('id')=='redio_0') c=0;
                var num1 = parseInt($('#single_score_'+c).val());
                $('#single_score_'+c).val(num1+num);
                $('#bureau_score_'+c).val(num+parseInt($('#bureau_score_'+c).val() ) );
            }
        });
        save_ajax(game_over);
    }
    ...
    function clickChange(game_over){
        ...
        $('.span_redio').each(function(){
            if($(this).css('display')=='none'){
                ...
                if($(this).attr('id')=='redio_0') ch=1;
                $('#ball_0').val(ch);
                $('#ball_1').val(1-ch);
                $('#ball_right_0').val(ch);
                $('#ball_right_1').val(1-ch);
            }
            else{
                $(this).css('display','none');
            }
        });
        $('#single_score_0').val(0);
        $('#single_score_1').val(0);
        save_ajax(game_over);
    }
    ...
    function save_ajax(game_over,n=0){
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo $this->createUrl('save_billiards_score',array('id'=>$model->id)); ?>&game_over='+game_over,
            data: form,
            dataType: 'json',
            success: function(data){
                if(data>0 && game_over==908 && n==1){
    ...
    function clickOver(n,game_over){
        var gl = parseInt($('#bureau_score_0').val());
        var gr = parseInt($('#bureau_score_1').val());
        ...
        save_ajax(game_over,n);
    }

    function clickRevoke(game_over){
        $.ajax({
            type: 'post',
            url: '<?php echo $this->createUrl('revoke',array('id'=>$model->id)); ?>',
            dataType: 'json',
            success: function(data){
                if(data!=''){
                    for(var i=0;i<2;i++){
                        $('#single_score_'+i).val(data['single_score_'+i]);
    ...
    function get_score_ajax(){
        $.ajax({
            type: 'get',
            url: '<?php echo $this->createUrl('getScore',array('id'=>$model->id,'bs'=>'1')); ?>',
            dataType: 'json',
            success: function(data){
                console.log(data);
                if(data!=''){
                    for(var i=0;i<data.length;i++){
                        var tid = $('#id_'+i).val();
                        if(data[i]['id']==tid){
                            $('#single_score_'+i).val(data[i]['single_score']);
