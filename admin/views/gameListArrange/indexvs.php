            ...
            <?php
                ...
                $text='';
                    for($c=1;$c<=$cols;$c++) {
                        ...
                        if($sd[2]){
                            $sd[0]='<p>'.$sd[0].'</p><p>'.$sd[1].'</p><p>'.$sd[2].'</p>';
                        }
                        $td_col = 'background-color:#fff;'; 
                        if($text){

                               if(!(($text>='0')&&($text<='9z'))){
                                   $td_col = 'background-color:#ffebcc;'; 
                                   $td_col = 'background-color:#beffbe;';
                                }
                            }
                echo '<td style="border:1px solid #000000;'.$td_col.'"><span style="float: left;">'.$sd[0].'</span><span style="float: right;">'.$sd[1].'</span></td>';
            }?>
            </tr>
        <?php } ?>
            </table>
...
<script>
    ...
    $('.sub').blur(function(){
        ...
        $.ajax({
            ...
            data: {form:form,filed:filed,colid:colid,arrid:arrid},
            dataType: 'json',
            success: function(data){
                if(data==1){
                    console.log('成功');
