    	...
    	<?php if($_REQUEST['game_id']<>0) { ?>
            <div class="gamesign-rt game_list_arrange">
                <div class="gamesign-group">
                    ...
                    <a <?php if($data_id==0){?> class="current"<?php }?> href="<?php echo $this->createUrl('gameSignList/event_mem_index', array('game_id'=>$_REQUEST['game_id'],'game_name'=>$_REQUEST['game_name']));?>">全部
                    </a>
       ...
       <?php } ?>
        <div class="<?php if($_REQUEST['game_id']<>0) { ?>gamesign-lt <?php } else { ?>box-content<?php } ?>" style=" border:none;">
            <div class="box-title c">
                ...
                <?php if ($_REQUEST['game_id']>0) { ?><span class="back"><a class="btn" href="javascript:;" onclick="we.back('<?php echo $this->createUrl('gameList/events_signin_index');?>');"><i class="fa fa-reply"></i>返回</a></span><?php } ?>
        </div><!--box-title end-->
        <div class="box-detail-tab box-detail-tab-green mt15">
            <ul class="c">
                <?php $action=Yii::app()->controller->getAction()->id;;?>
                <li><a href="<?php echo $this->createUrl('gameTeamTable/event_mem_index',array('game_id'=>$_REQUEST['game_id'],'data_type'=>$_REQUEST['data_type'],'game_name'=>$_REQUEST['game_name']));?>">团队</a></li>
                <li class="current"><a href="<?php echo $this->createUrl('gameSignList/event_mem_index',array('game_id'=>$_REQUEST['game_id'],'data_type'=>$_REQUEST['data_type'],'game_name'=>$_REQUEST['game_name']));?>">个人</a></li>
            </ul>
...
<?php $index = 1; foreach($arclist as $v){ ?>
                    <tr>
                        <td class="check check-item" style="text-align: center;">
...
<script>
    ...
    function addRegister(id){
        $.ajax({
            type: 'post',
            url: '<?php echo $this->createUrl('add_register');?>',
            data: {id: id},
            dataType: 'json',
