 ...
 <table>
    <tr>
        ...
        <td width="30%" style="text-align:center;">服务信息</td>
        <td width="15%" style="text-align:center;">服务类别</td>
        <td width="15%" style="text-align:center;">资源等级</td>
        <td width="15%" style="text-align:center;">服务项目</td>
...
<?php if(!empty($model->set_list)) foreach ($model->set_list as $v) { ?>
 <tr data-server="<?php echo $v->server_sourcer_id;?>">
   ...
   <td width="30%" title="<?php echo $v->s_name; ?>"><input type="hidden" name="product[<?php echo $v->id;?>][id]" value="<?php echo $v->id;?>" /><?php echo $v->s_code; ?>-<?php echo $v->s_name; ?>
...
<input type="hidden" name="product[<?php echo $v->id;?>][server_sourcer_id]" value="<?php echo $v->server_sourcer_id;?>" /></td>
 ...
 <?php if(!empty($v->project_ids)) $project=ProjectList::model()->findAll('id in('.$v->project_ids.')'); ?>
   <td width="15%"><?php if(!empty($project))foreach($project as $p)   echo $p->project_name; ?></td>  
   <td width="15%"><a class="btn" href="javascript:;" onclick="fnMemberprice(<?php echo $v->id;?>);" title="可约服务明细">可约服务明细</a>&nbsp;<a class="btn" href="javascript:;" onclick="fnDeleteProduct(this);" title="删除"><i class="fa fa-trash-o"></i></a></td>
   </tr>
 <script>
...
<script>
	...
	var num=oldnum+1;
	 $('#product_select_btn').on('click', function(){
         ...
         var stype = $('#QmddServerSetInfo_t_stypeid').val();
		 var html_str='';
		if (club_id=='') {
        ...
        if (stype=='') {
            we.msg('minus','请先选择服务类别');
            return false;
        }
		
        $.dialog.data('product_id', 0);
        $.dialog.open('<?php echo $this->createUrl("select/server_sourcer");?>&club_id='+club_id+'&type='+type+'&stype='+stype,{
            id:'fuwuziyuan',
            lock:true,
            opacity:0.3,
            title:'选择服务资源',
            ...
            height:'80%',
            close: function () {
            if($.dialog.data('product_id')!=''){
				s_no++;
				html_str = html_str + '<tr data-server="'+$.dialog.data('product_id')+'">'+
                '<td width="10%" style="text-align:center;">1</td>'+
				'<td width="30%">'+$.dialog.data('product_code')+'-'+$.dialog.data('product_name')+
				'<input type="hidden" class="input-text" name="product['+num+'][id]" value="null" />'+
                '<input type="hidden" class="input-text" name="product['+num+'][server_sourcer_id]" value="'+$.dialog.data('product_id')+'" /></td>'+
				'<td width="15%">'+$.dialog.data('s_type')+'</td>'+
   				'<td width="15%">'+$.dialog.data('level')+'</td>'+
                '<td width="15%">'+$.dialog.data('project')+'</td>'+
				'<td width="15%"><a class="btn" href="javascript:;" onclick="fnDeleteProduct(this);" title="删除">'
