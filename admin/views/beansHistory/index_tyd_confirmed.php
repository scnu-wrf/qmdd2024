...
<div class="box">
    <div class="box-title c">
        <h1>
            当前界面：系统 》积分/体育豆 》积分兑换体育豆确认<?php echo !empty($_REQUEST['state'])?' 》待确认':'';?>
        </h1>
    ...
    <div class="box-content">
    	<div class="box-header">
            ...
            <span class="exam" onclick="on_exam();"><p>待核对：<span style="color:red;font-weight: bold;"><?php echo $count1; ?></span></p></span>
            <?php }else{?>
                <a class="btn btn-blue" href="javascript:;" onclick="checkval('.check-item input:checked');">核对</a>
            <?php }?>
        </div><!--box-header end-->
                ...
                <table class="list">
                    ...
                    <?php $index = 1; foreach($arclist as $v){ ?>
                        <tr>
                            ...
                            <td><?php if(!empty($v->gfcredit))echo $v->gfcredit->credit; ?></td>
                            <td><?php echo $v->got_beans_num; ?></td>
                            <?php if(empty($_REQUEST['state'])){?>
                                <td><?php echo $v->admin_gfnick; ?></td>
                                <td><?php echo $v->exchange_time; ?></td>
                            <?php }else{?>
                                <td><a class="btn btn-blue" href="javascript:;" onclick="checkval(<?=$v->id;?>,'one')">核对</a></td>
                            <?php }?>
                        </tr>
                    <?php $index++; } ?>
                </table>
