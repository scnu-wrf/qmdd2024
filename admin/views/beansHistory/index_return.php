...
<div class="box">
    <div class="box-title c">
        <h1>
        当前界面：系统 》体育豆 》<?php echo !empty($_REQUEST['state'])?'体育豆退还审核 》待审核':'体育豆退还处理';?>
        </h1>
    ...
    <div class="box-content">
    	<div class="box-header">
            ...
            <?php }else{?>
                <a class="btn btn-blue" href="javascript:;" onclick="checkval('.check-item input:checked');">审核</a>
            <?php }?>
        </div><!--box-header end-->
                ...
                <table class="list">
                    ...
                    <?php $index = 1; foreach($arclist as $v){ ?>
                        <tr>
                            ...
                            <td>
                                <?php 
                                    $h=BeansHistory::model()->find('object=1480 and order_num="'.$v->order_num.'"');
                                    echo !empty($h)?$h->consume_beans_num:""; 
                                ?>
                            </td>
                            <td><?php echo $v->source_ske; ?></td>
                            ...
                            <?php }else{?>
                                <td><a class="btn btn-blue" href="javascript:;" onclick="checkval(<?=$v->id;?>,'one')">确认审核</a></td>
                            <?php }?>
                        </tr>
                    <?php $index++; } ?>
                </table>
...
<script>
    ...
    function confirmed(id){
        ...
        $.ajax({
            type:'post',
            url:'<?php echo $this->createUrl('confirmed'); ?>&id='+id+'&al=tongguo',
            // data:{id:id},
            dataType:'json',
