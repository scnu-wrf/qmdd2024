...
<div class="box">
    <div id="t0" class="box-title c">
        <h1>当前界面：赛事 》赛事发布 》发布赛事 》<a class="nav-a"><?php echo $model->game_title; echo (empty($model->id)) ? '添加' : '/详情'; ?></a></h1>
            ...
            <div style="display:block;" class="box-detail-tab-item">
            	...
            	<table id="game_info" data-num="new">
					...
					<tr>
                        ...
                        <td colspan=2>
                            <?php echo $form->dropDownList($model, 'game_area', Chtml::listData(BaseCode::model()->getCode(158,' order by F_CODE'), 'f_id', 'F_NAME'), array('prompt'=>'请选择')); ?>
				...
				<table id="intro_table" data-num="new">
					...
					<tr>
						...
						<td><a class="btn" href="javascript:;" onclick="">预览</a> <a class="btn" href="javascript:;" onclick="fnDeleteIntro(this);">删除</a></td>
					</tr>
						<?php }?>
					<?php }?>
				</table>
                    ...
                    <tr>
                        ...
                        <td id="dpic_game_small_pic">
                            ...
                            <?php }?>
                                ...
                                <script>we.uploadpic('<?php echo get_class($model);?>_game_small_pic','<?php echo $picprefix;?>');</script>
                            <?php echo $form->error($model, 'game_small_pic', $htmlOptions = array()); ?>
                            <sub>注：规格530px*530px</sub>
                        </td>
                        ...
                        <td colspan="3">
                            ...
                            <script>we.uploadpic('<?php echo get_class($model);?>_game_big_pic','<?php echo $picprefix;?>','','',function(e1,e2){fnscrollPic(e1.savename,e1.allpath);},50);</script>
                            <?php echo $form->error($model, 'game_big_pic', $htmlOptions = array()); ?>
                            <sub>注：1-5张，规格1080px*809px</sub>
                        </td>
                    </tr>
                    ...
                    <tr id="dis_sh">
                        ...
                        <td>
                            <?php
                                ...
                                $list_data = GameListData::model()->count('game_id='.$model->id);
                                if($model->state!=372 && $model->state!=373){
                                    echo show_shenhe_box(array('baocun'=>'保存'));
                                    if(!empty($model->id) && $list_data>0){
