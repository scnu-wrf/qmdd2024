...
<script>
                ...
                '<form id="add_form" name="add_form">'+
                    '<table class="list" id="table_tag" style="width:100%;border: solid 1px #d9d9d9;">'+
                        '<tr>'+
                            ...
                            '<td style="width:40%;">'+
                                '<span id="game_box"></span>'+
                                '<input id="game_id" name="game_id" type="hidden">'+
                                '<input id="game_btn" class="btn" type="button" value="选择">'+
                            '</td>'+
                        '</tr>'+
                        '<tr>'+
    ...
    $(document).on('click','#game_btn', function(){
        ...
        $.dialog.open('<?php echo $this->createUrl("select/video_game_list");?>&club_id='+club_id,{
            id:'saishi',
            lock:true,
            opacity:0.3,
            title:'选择赛事列表数据',
            ...
            close: function () {
                if($.dialog.data('game_id')>0){
                    ...
                    $("#game_id").val($.dialog.data('game_id'));
