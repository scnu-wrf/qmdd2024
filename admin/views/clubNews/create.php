             ...
             <input type="hidden" name="news_type" value="<?php echo $_REQUEST['news_type'];?>">
                <table class="mt15">
                    ...
                    <tr id='show_video_line'  style="display:none;"><!--news_type=227时显示,此外表，链接club_news_pic表-->
                        ...
                        ?></td>
                        <td colspan="3" style="padding:30px 15px;" class="up_btn">
...
background-image:linear-gradient(45deg,rgba(255,255,255,0.15) 25%,transparent 25%,transparent 50%,rgba(255,255,255,0.15) 50%,rgba(255,255,255,0.15) 75%,transparent 75%,transparent);background-size:40px 40px;height: 100%;color: #fff;text-align: right;padding-right: 5px;box-sizing: border-box;" progress="0"></div>
							...
							<?php if($model->gf_material!=null){?>
								...
								<a href="../qmdd2018/index.php?r=videoLivePrograms/video_player&url=<?php echo $model->gf_material->v_file_path.$model->gf_material->v_name;?>" target="_blank">
							...
							<div class="upload fl">
								<input name="fileCode" id="fileCode" value="38_cn" type="hidden" />
								<script>we.materialVideo(function(data){ $('#ClubNews_news_video').val(data.id).trigger('blur');},'上传',"<?php echo $this->createUrl('GfMaterial/saveMaterial');?>");</script>
							</div>
