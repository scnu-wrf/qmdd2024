<div class="box">
    <div class="box-title c">
        <h1>当前界面：商城》商品上架》<a class="nav-a">库存及销售明细</a></h1>
            ...
            <table class="list">
                <thead>
                    <tr>
                        ...
                        <th><?php echo $model->getAttributeLabel('secondary_quantity');?></th>
...
if (is_array($arclist)) foreach($arclist as $v){ ?>
                    ...
                    <?php $record=MallPricingDetails::model()->find('(set_details_id='.$v->id.') order by id DESC'); ?>
                        ...
                        if($v->sale_id==4){
                            $txt='(导购)';
                        } else if($v->sale_id==5) $txt='(二次上架)';
                        ?>
                        <td><?php if($v->sale_id==4 || $v->sale_id==5) echo $v->secondary_quantity.$txt; ?></td>
